        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('reference')
                    ...
                    ->formatStateUsing(fn (StockMovement $record): string => match (true) {
                        ...
                        $record->stock_adjustment_id && $record->stockAdjustment
                            => $record->stockAdjustment->code,
                        default => '-',
                    }),
            ...
            ->recordActions([
                Action::make('detail')
                    ->label('Detail')
                    ->icon('heroicon-o-eye')
                    ->url(fn (StockMovement $record): ?string => match (true) {
                        $record->movement_type === 'sale' && $record->order_id
                            => OrderResource::getUrl('view', ['record' => $record->order_id]),
                        $record->stock_adjustment_id
                            => StockAdjustmentResource::getUrl('edit', ['record' => $record->stock_adjustment_id]),
                        default => null,
                    }),
            ])

...
class CashierDashboardController extends Controller
{
    public function index()
    {
        ...
        $stats = Cache::remember("dashboard_stats_{$todayKey}", 30, fn() =>
        ...
        $pesananAktif = Cache::remember("dashboard_aktif_{$todayKey}", 15, fn() =>
            Order::where('status', '!=', Order::STATUS_SELESAI)
                ->where(fn($q) =>
        ...
        $transaksiTerbaru = Cache::remember("dashboard_terbaru_{$todayKey}", 15, fn() =>
            Order::with(['items' => fn($q) => $q->select('id', 'order_id', 'menu_id', 'quantity')->with(['menu' => fn($q) => $q->select('id', 'name')])])
